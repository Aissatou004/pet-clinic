...
import { sampleWithRequiredData } from '../clinique.test-samples';
...
describe('Clinique Management Component', () => {
  ...
  let comp: Clinique;
  let fixture: ComponentFixture<Clinique>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Clinique);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 21629 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.cliniques()[0]).toEqual(expect.objectContaining({ id: 21629 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 21629 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.cliniques()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 21629 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to cliniqueService', () => {
      ...
      vitest.spyOn(service, 'getCliniqueIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCliniqueIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.cliniquesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
