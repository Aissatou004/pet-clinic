...
import { sampleWithRequiredData } from '../peser-animal.test-samples';
import { PeserAnimalService } from '../service/peser-animal.service';

import { PeserAnimal } from './peser-animal';

vitest.useFakeTimers();

describe('PeserAnimal Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PeserAnimal;
  let fixture: ComponentFixture<PeserAnimal>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PeserAnimal);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 12626 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.peserAnimals()[0]).toEqual(expect.objectContaining({ id: 12626 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 12626 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.peserAnimals()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 12626 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to peserAnimalService', () => {
      const entity = { id: 12626 };
      vitest.spyOn(service, 'getPeserAnimalIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPeserAnimalIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.peserAnimalsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
