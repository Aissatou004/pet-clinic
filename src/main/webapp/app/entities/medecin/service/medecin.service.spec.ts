...
import { sampleWithFullData, sampleWithNewData, sampleWithPartialData, sampleWithRequiredData } from '../medecin.test-samples';

import { MedecinService } from './medecin.service';

const requireRestSample: IMedecin = {
...
describe('Medecin Service', () => {
  ...
  describe('Service methods', () => {
    ...
    it('should create a Medecin', () => {
      const medecin = { ...sampleWithNewData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.create(medecin).subscribe(resp => (expectedResult = resp));
    ...
    it('should update a Medecin', () => {
      const medecin = { ...sampleWithRequiredData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.update(medecin).subscribe(resp => (expectedResult = resp));
    ...
    describe('addMedecinToCollectionIfMissing', () => {
      it('should add a Medecin to an empty array', () => {
        const medecin: IMedecin = sampleWithRequiredData;
        expectedResult = service.addMedecinToCollectionIfMissing([], medecin);
        expect(expectedResult).toEqual([medecin]);
      });
      ...
      it("should add a Medecin to an array that doesn't contain it", () => {
        const medecin: IMedecin = sampleWithRequiredData;
        const medecinCollection: IMedecin[] = [sampleWithPartialData];
        expectedResult = service.addMedecinToCollectionIfMissing(medecinCollection, medecin);
        expect(expectedResult).toHaveLength(2);
      ...
      it('should add only unique Medecin to an array', () => {
        const medecinArray: IMedecin[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const medecinCollection: IMedecin[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const medecin: IMedecin = sampleWithRequiredData;
        const medecin2: IMedecin = sampleWithPartialData;
        expectedResult = service.addMedecinToCollectionIfMissing([], medecin, medecin2);
        expect(expectedResult).toEqual([medecin, medecin2]);
      });
    ...
    describe('compareMedecin', () => {
      it('should return true if both entities are null', () => {
        ...
        const compareResult = service.compareMedecin(entity1, entity2);

        expect(compareResult).toEqual(true);
      });

      it('should return false if one entity is null', () => {
        const entity1 = { id: 19080 };
        const entity2 = null;

        const compareResult1 = service.compareMedecin(entity1, entity2);
        const compareResult2 = service.compareMedecin(entity2, entity1);
